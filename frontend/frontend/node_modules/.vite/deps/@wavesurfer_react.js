...
function n(t3, e3) {
  ...
  for (const [t4, s2] of Object.entries(e3))
    if ("children" === t4)
      for (const [t5, s3] of Object.entries(e3))
        "string" == typeof s3 ? i3.appendChild(document.createTextNode(s3)) : i3.appendChild(n(t5, s3));
    else
      "style" === t4 ? Object.assign(i3.style, s2) : "textContent" === t4 ? i3.textContent = s2 : i3.setAttribute(t4, s2.toString());
  return i3;
}
function s(t3, e3, i3) {
  const s2 = n(t3, e3 || {});
...
var o = { fetchBlob: function(e3, i3, n3) {
  return t(this, void 0, void 0, function* () {
    ...
    if (s2.status >= 400)
      throw new Error(`Failed to fetch ${e3}: ${s2.status} (${s2.statusText})`);
    return function(e4, i4) {
      t(this, void 0, void 0, function* () {
        if (!e4.body || !e4.headers)
          return;
        const n4 = e4.body.getReader(), s3 = Number(e4.headers.get("Content-Length")) || 0;
        let r3 = 0;
        const o3 = (e5) => t(this, void 0, void 0, function* () {
          r3 += (null == e5 ? void 0 : e5.length) || 0;
          const t3 = Math.round(r3 / s3 * 100);
          i4(t3);
...
var a = class extends e {
  ...
  setSrc(t3, e3) {
    if (this.getSrc() === t3)
      return;
    this.revokeSrc();
    const i3 = e3 instanceof Blob && this.canPlayType(e3.type) ? URL.createObjectURL(e3) : t3;
    this.media.src = i3;
  }
  destroy() {
...
var h = class _h extends e {
  constructor(t3, e3) {
    super(), this.timeouts = [], this.isScrollable = false, this.audioData = null, this.resizeObserver = null, this.lastContainerWidth = 0, this.isDragging = false, this.subscriptions = [], this.subscriptions = [], this.options = t3;
  ...
  parentFromOptionsContainer(t3) {
    let e3;
    if ("string" == typeof t3 ? e3 = document.querySelector(t3) : t3 instanceof HTMLElement && (e3 = t3), !e3)
      throw new Error("Container not found");
    return e3;
  }
  initEvents() {
    const t3 = (t4) => {
      const e4 = this.wrapper.getBoundingClientRect(), i3 = t4.clientX - e4.left, n3 = t4.clientY - e4.top;
      return [i3 / e4.width, n3 / e4.height];
    };
    this.wrapper.addEventListener("click", (e4) => {
      const [i3, n3] = t3(e4);
      this.emit("click", i3, n3);
    }), this.wrapper.addEventListener("dblclick", (e4) => {
      const [i3, n3] = t3(e4);
      ...
      const { scrollLeft: t4, scrollWidth: e4, clientWidth: i3 } = this.scrollContainer, n3 = t4 / e4, s2 = (t4 + i3) / e4;
      this.emit("scroll", n3, s2);
    });
    const e3 = this.createDelay(100);
    this.resizeObserver = new ResizeObserver(() => {
      e3().then(() => this.onContainerResize()).catch(() => {
      });
  ...
  initDrag() {
    this.subscriptions.push(function(t3, e3, i3, n3, s2 = 3, r3 = 0, o3 = 100) {
      if (!t3)
        return () => {
        };
      ...
      const l3 = (l4) => {
        ...
        const p2 = Date.now(), m = (n4) => {
          if (n4.preventDefault(), n4.stopPropagation(), a3 && Date.now() - p2 < o3)
            return;
          const r4 = n4.clientX, h3 = n4.clientY, l5 = r4 - d2, m2 = h3 - c3;
          if (u3 || Math.abs(l5) > s2 || Math.abs(m2) > s2) {
            const n5 = t3.getBoundingClientRect(), { left: s3, top: o4 } = n5;
            u3 || (null == i3 || i3(d2 - s3, c3 - o4), u3 = true), e3(l5, m2, r4 - s3, h3 - o4), d2 = r4, c3 = h3;
          }
        }, f2 = (e4) => {
          if (u3) {
  ...
  initHtml() {
    const t3 = document.createElement("div"), e3 = t3.attachShadow({ mode: "open" });
    return e3.innerHTML = `
      <style>
        :host {
          user-select: none;
  ...
  destroy() {
    var t3;
    this.subscriptions.forEach((t4) => t4()), this.container.remove(), null === (t3 = this.resizeObserver) || void 0 === t3 || t3.disconnect();
  }
  createDelay(t3 = 10) {
    let e3, i3;
    const n3 = () => {
  ...
  convertColorValues(t3) {
    ...
    if (t3.length < 2)
      return t3[0] || "";
    const e3 = document.createElement("canvas"), i3 = e3.getContext("2d"), n3 = e3.height * (window.devicePixelRatio || 1), s2 = i3.createLinearGradient(0, 0, 0, n3), r3 = 1 / (t3.length - 1);
    return t3.forEach((t4, e4) => {
      const i4 = e4 * r3;
      s2.addColorStop(i4, t4);
    }), s2;
  }
  renderBarWaveform(t3, e3, i3, n3) {
    const s2 = t3[0], r3 = t3[1] || t3[0], o3 = s2.length, { width: a3, height: h2 } = i3.canvas, l3 = h2 / 2, d2 = window.devicePixelRatio || 1, c3 = e3.barWidth ? e3.barWidth * d2 : 1, u3 = e3.barGap ? e3.barGap * d2 : e3.barWidth ? c3 / 2 : 0, p2 = e3.barRadius || 0, m = a3 / (c3 + u3) / o3, f2 = p2 && "roundRect" in i3 ? "roundRect" : "rect";
    i3.beginPath();
    let g = 0, v = 0, b = 0;
    for (let t4 = 0; t4 <= o3; t4++) {
      const o4 = Math.round(t4 * m);
      if (o4 > g) {
        const t5 = Math.round(v * l3 * n3), s3 = t5 + Math.round(b * l3 * n3) || 1;
        let r4 = l3 - t5;
        "top" === e3.barAlign ? r4 = 0 : "bottom" === e3.barAlign && (r4 = h2 - s3), i3[f2](g * (c3 + u3), r4, c3, s3, p2), g = o4, v = 0, b = 0;
      }
      const a4 = Math.abs(s2[t4] || 0), d3 = Math.abs(r3[t4] || 0);
      a4 > v && (v = a4), d3 > b && (b = d3);
    }
  ...
  renderLineWaveform(t3, e3, i3, n3) {
    const s2 = (e4) => {
      const s3 = t3[e4] || t3[0], r3 = s3.length, { height: o3 } = i3.canvas, a3 = o3 / 2, h2 = i3.canvas.width / r3;
      i3.moveTo(0, a3);
      let l3 = 0, d2 = 0;
      for (let t4 = 0; t4 <= r3; t4++) {
        const r4 = Math.round(t4 * h2);
        if (r4 > l3) {
          const t5 = a3 + (Math.round(d2 * a3 * n3) || 1) * (0 === e4 ? -1 : 1);
          i3.lineTo(l3, t5), l3 = r4, d2 = 0;
        }
        const o4 = Math.abs(s3[t4] || 0);
        o4 > d2 && (d2 = o4);
      }
      i3.lineTo(l3, a3);
    };
    i3.beginPath(), s2(0), s2(1), i3.fill(), i3.closePath();
  }
  renderWaveform(t3, e3, i3) {
    if (i3.fillStyle = this.convertColorValues(e3.waveColor), e3.renderFunction)
      return void e3.renderFunction(t3, i3);
    let n3 = e3.barHeight || 1;
    if (e3.normalize) {
      const e4 = Array.from(t3[0]).reduce((t4, e5) => Math.max(t4, Math.abs(e5)), 0);
      n3 = e4 ? 1 / e4 : 1;
    }
    e3.barWidth || e3.barGap || e3.barAlign ? this.renderBarWaveform(t3, e3, i3, n3) : this.renderLineWaveform(t3, e3, i3, n3);
  }
  renderSingleCanvas(t3, e3, i3, n3, s2, r3, o3) {
    const a3 = window.devicePixelRatio || 1, h2 = document.createElement("canvas");
  ...
  renderMultiCanvas(e3, i3, n3, s2, r3, o3) {
    return t(this, void 0, void 0, function* () {
      const a3 = window.devicePixelRatio || 1, l3 = n3 / a3;
      let d2 = Math.min(_h.MAX_CANVAS_WIDTH, this.scrollContainer.clientWidth);
      if (i3.barWidth || i3.barGap) {
        const t3 = i3.barWidth || 0.5, e4 = t3 + (i3.barGap || t3 / 2);
        d2 % e4 != 0 && (d2 = Math.floor(d2 / e4) * e4);
      }
      const c3 = (t3) => {
        const n4 = t3 * d2, a4 = Math.min(l3 - n4, d2), h2 = e3.map((t4) => {
          const e4 = Math.floor(n4 / l3 * t4.length), i4 = Math.floor((n4 + a4) / l3 * t4.length);
          return t4.slice(e4, i4);
        });
        this.renderSingleCanvas(h2, i3, a4, s2, n4, r3, o3);
      }, u3 = Math.ceil(l3 / d2);
      if (1 === u3)
        return void c3(0);
      const p2 = this.scrollContainer.scrollLeft / l3, m = Math.floor(p2 * u3);
      c3(m), c3(m + 1), yield Promise.all([(() => t(this, void 0, void 0, function* () {
        const t3 = this.createDelay();
        for (let e4 = m - 1; e4 >= 0; e4--)
          yield t3(), c3(e4);
      }))(), (() => t(this, void 0, void 0, function* () {
        const t3 = this.createDelay();
        for (let e4 = m + 2; e4 < u3; e4++)
          yield t3(), c3(e4);
      }))()]);
    });
  }
  renderChannel(e3, i3, n3, s2) {
    return t(this, void 0, void 0, function* () {
      var { overlay: t3 } = i3, r3 = function(t4, e4) {
        var i4 = {};
        for (var n4 in t4)
        ...
        if (null != t4 && "function" == typeof Object.getOwnPropertySymbols) {
          var s3 = 0;
          for (n4 = Object.getOwnPropertySymbols(t4); s3 < n4.length; s3++)
            e4.indexOf(n4[s3]) < 0 && Object.prototype.propertyIsEnumerable.call(t4, n4[s3]) && (i4[n4[s3]] = t4[n4[s3]]);
        }
        return i4;
      }(i3, ["overlay"]);
      const o3 = document.createElement("div"), a3 = this.getHeight(r3.height, r3.splitChannels);
      o3.style.height = `${a3}px`, t3 && s2 > 0 && (o3.style.marginTop = `-${a3}px`), this.canvasWrapper.style.minHeight = `${a3}px`, this.canvasWrapper.appendChild(o3);
      const h2 = o3.cloneNode();
      this.progressWrapper.appendChild(h2), yield this.renderMultiCanvas(e3, r3, n3, a3, o3, h2);
    });
  }
  render(e3) {
    return t(this, void 0, void 0, function* () {
      ...
      const t3 = window.devicePixelRatio || 1, i3 = this.scrollContainer.clientWidth, n3 = Math.ceil(e3.duration * (this.options.minPxPerSec || 0));
      this.isScrollable = n3 > i3;
      const s2 = this.options.fillParent && !this.isScrollable, r3 = (s2 ? i3 : n3) * t3;
      this.wrapper.style.width = s2 ? "100%" : `${n3}px`, this.scrollContainer.style.overflowX = this.isScrollable ? "auto" : "hidden", this.scrollContainer.classList.toggle("noScrollbar", !!this.options.hideScrollbar), this.cursor.style.backgroundColor = `${this.options.cursorColor || this.options.progressColor}`, this.cursor.style.width = `${this.options.cursorWidth}px`, this.audioData = e3, this.emit("render");
      try {
        if (this.options.splitChannels)
          yield Promise.all(Array.from({ length: e3.numberOfChannels }).map((t4, i4) => {
            var n4;
            const s3 = Object.assign(Object.assign({}, this.options), null === (n4 = this.options.splitChannels) || void 0 === n4 ? void 0 : n4[i4]);
            return this.renderChannel([e3.getChannelData(i4)], s3, r3, i4);
          }));
        else {
          const t4 = [e3.getChannelData(0)];
          e3.numberOfChannels > 1 && t4.push(e3.getChannelData(1)), yield this.renderChannel(t4, this.options, r3, 0);
        }
      } catch (t4) {
        return;
      }
  ...
  scrollIntoView(t3, e3 = false) {
    ...
    {
      const t4 = this.scrollContainer.scrollLeft, e4 = t4 / n3, i4 = (t4 + s2) / n3;
      this.emit("scroll", e4, i4);
    }
  }
...
var d = class extends e {
  ...
  set src(t3) {
    if (this.currentSrc = t3, this._duration = void 0, !t3)
      return this.buffer = null, void this.emit("emptied");
    fetch(t3).then((e3) => {
      if (e3.status >= 400)
        throw new Error(`Failed to fetch ${t3}: ${e3.status} (${e3.statusText})`);
  ...
  stopAt(t3) {
    var e3, i3;
    const n3 = t3 - this.currentTime;
    null === (e3 = this.bufferNode) || void 0 === e3 || e3.stop(this.audioContext.currentTime + n3), null === (i3 = this.bufferNode) || void 0 === i3 || i3.addEventListener("ended", () => {
  ...
  getChannelData() {
    ...
    for (let i3 = 0; i3 < e3; i3++)
      t3.push(this.buffer.getChannelData(i3));
    return t3;
  }
};
...
var u = class _u extends a {
  ...
  constructor(t3) {
    ...
    Promise.resolve().then(() => {
      ...
      (n3 || t4 && e4) && this.load(n3, t4, e4).catch(() => null);
    });
  }
  ...
  initRendererEvents() {
    ...
    }), this.renderer.on("scroll", (t3, e3) => {
      const i3 = this.getDuration();
      this.emit("scroll", t3 * i3, e3 * i3);
  ...
  setOptions(t3) {
    this.options = Object.assign({}, this.options, t3), this.renderer.setOptions(this.options), t3.audioRate && this.setPlaybackRate(t3.audioRate), null != t3.mediaControls && (this.getMediaElement().controls = t3.mediaControls);
  }
  registerPlugin(t3) {
  ...
  loadAudio(e3, n3, s2, r3) {
    return t(this, void 0, void 0, function* () {
      var t3;
      if (this.emit("load", e3), !this.options.media && this.isPlaying() && this.pause(), this.decodedData = null, !n3 && !s2) {
        ...
        const s3 = (t4) => this.emit("loading", t4);
        n3 = yield o.fetchBlob(e3, s3, i3);
      }
      this.setSrc(e3, n3);
      const a3 = r3 || this.getDuration() || (yield new Promise((t4) => {
        this.onMediaEvent("loadedmetadata", () => t4(this.getDuration()), { once: true });
      }));
      if (!e3 && !n3) {
      ...
      if (s2)
        this.decodedData = i.createBuffer(s2, a3 || 0);
      else if (n3) {
  ...
  loadBlob(e3, i3, n3) {
    return t(this, void 0, void 0, function* () {
      try {
        return yield this.loadAudio("blob", e3, i3, n3);
      } catch (t3) {
        throw this.emit("error", t3), t3;
      }
    });
...
var l2 = (0, import_react.memo)((e3) => {
  const r3 = (0, import_react.useRef)(null), [c3, s2] = function(t3) {
    return (0, import_react.useMemo)(() => {
      ...
      for (const t4 in e4)
        u2(t4) ? delete e4[t4] : delete n3[t4];
      return [e4, n3];
    }, [t3]);
  }(e3);
...
function p(t3) {
  ...
  const n3 = i2(e3, function(t4, e4) {
    var n4 = {};
    for (var r3 in t4)
      Object.prototype.hasOwnProperty.call(t4, r3) && e4.indexOf(r3) < 0 && (n4[r3] = t4[r3]);
    if (null != t4 && "function" == typeof Object.getOwnPropertySymbols) {
      var o3 = 0;
      for (r3 = Object.getOwnPropertySymbols(t4); o3 < r3.length; o3++)
        e4.indexOf(r3[o3]) < 0 && Object.prototype.propertyIsEnumerable.call(t4, r3[o3]) && (n4[r3[o3]] = t4[r3[o3]]);
    }
    return n4;
